...
CLASS lcl_class IMPLEMENTATION.
  ...
  METHOD get_data.
    SELECT uagaci~ana_malzeme,
      MIN (
            ( CAST ( stok~stok_miktari AS FLTP ) / CAST ( uagaci~alt_malzeme_miktari AS FLTP )
          )
      ) AS minstok
      FROM zko_egt_t_uagaci AS uagaci
      INNER JOIN zko_egt_t_stok AS stok ON stok~alt_malzeme EQ uagaci~altmalzeme
