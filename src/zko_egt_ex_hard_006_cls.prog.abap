...
CLASS lcl_class IMPLEMENTATION.
  ...
  METHOD siparis_ver.
    ...
    IF sy-subrc EQ 0
      ...
      AND gb_labst IS NOT INITIAL.
      go_main->insert_sip( ).
    ENDIF.
  ENDMETHOD.

  METHOD insert_sip.
    DATA: ls_data TYPE zko_t_spris.
    go_main->get_vbeln(
    IMPORTING
      ev_next_number = gv_vblen
      ).
    ls_data-vbeln = gv_vbeln.
    ls_data-matnr = gv_matnr.
    ls_data-labst = gv_labst.
    ls_data-cname = gv_uname.
    ls_data-cdate = gv_datum.
    ls_data-ctime = gv_uzeit.

    INSERT zko_t_spris FROM ls_data.
    go_main->display_alv( ).
  ENDMETHOD.
  ...
  METHOD set_fcat.
    CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
      EXPORTING
        i_structure_name = 'ZKO_S_ALV_SIPARIS_BILGI'
      CHANGING
        ct_fieldcat      = gt_fcat.
  ENDMETHOD.
